...
<body>
  ...
  <div class="text-container">
    <p class="custom-text">Content List</p> 
    <div class="d-flex">
        <form action="/content/search" method="GET" class="d-flex">
            <input type="text" name="search" class="form-control search-input" placeholder="Search by name" aria-label="Search" >
          </form>
      </form>
      <a href="{{ route('content.add') }}" class="btn add-content-btn ms-2">+ Add Content</a>
        </div>
      </div>
  </div>
  </div>  
  <div class="card-body table-responsive p-0">
    <table class="table table-hover text-nowrap">
        ...
        <tbody>
            ...
            <td>{{$item->status}}</td>
            <td>
                <a href="{{ route('content.edit', $item->id) }}" class="btn btn custom-btn">Edit</a></a>
            </td>
        </tr> 
        @endforeach
        </tbody>  
